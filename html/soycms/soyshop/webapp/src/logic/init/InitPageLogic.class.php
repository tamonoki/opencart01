...
class InitPageLogic extends SOY2LogicBase{
    ...
    function initPage() {
		...
		try{
			...
			$page->setName("トップページ");
			$page->setUri(SOYSHOP_TOP_PAGE_MARKER);
			$page->setType("complex");
			$id = $logic->create($page);

			//一覧
			$page = new SOYShop_Page();
			$page->setName("商品一覧");
			$page->setUri("item/list");
			$page->setType("list");
			$id = $logic->create($page);
				$page->setId($id);
			...
			$page = new SOYShop_Page();
			$page->setName("会社概要");
			$page->setUri("concept.html");
			$page->setType("free");
			$id = $logic->create($page);
				$page->setId($id);
				$freePage = $page->getPageObject();
				$freePage->setTitle("会社概要");
				$freePage->setContent(file_get_contents(dirname(__FILE__) . "/free/".SOYSHOP_TEMPLATE_ID."/concept.html"));
				$page->setPageObject($freePage);
				$logic->updatePageObject($page);

			//よくあるご質問
			$page = new SOYShop_Page();
			$page->setName("よくあるご質問");
			$page->setUri("question.html");
			$page->setType("free");
			$id = $logic->create($page);
				$page->setId($id);
				$freePage = $page->getPageObject();
				$freePage->setTitle("よくあるご質問");
				$freePage->setContent(file_get_contents(dirname(__FILE__) . "/free/".SOYSHOP_TEMPLATE_ID."/question.html"));
				$page->setPageObject($freePage);
				$logic->updatePageObject($page);
			...
			$page = new SOYShop_Page();
			$page->setName("お問い合わせ");
			$page->setUri("inquiry.html");
			$page->setType("free");
			$id = $logic->create($page);
				$page->setId($id);
				$freePage = $page->getPageObject();
				$freePage->setTitle("お問い合わせ");
				$freePage->setContent(file_get_contents(dirname(__FILE__) . "/free/".SOYSHOP_TEMPLATE_ID."/inquiry.html"));
				$page->setPageObject($freePage);
				$logic->updatePageObject($page);

			//プライバシーポリシー
			$page = new SOYShop_Page();
			$page->setName("プライバシーポリシー");
			$page->setUri("policy.html");
			$page->setType("free");
			$id = $logic->create($page);
				$page->setId($id);
				$freePage = $page->getPageObject();
				$freePage->setTitle("プライバシーポリシー");
				$freePage->setContent(file_get_contents(dirname(__FILE__) . "/free/".SOYSHOP_TEMPLATE_ID."/policy.html"));
				$page->setPageObject($freePage);
				$logic->updatePageObject($page);

			//お支払いについて
			$page = new SOYShop_Page();
			$page->setName("お支払い・配送について");
			$page->setUri("delivery.html");
			$page->setType("free");
			$id = $logic->create($page);
				$page->setId($id);
				$freePage = $page->getPageObject();
				$freePage->setTitle("お支払い・配送について");
				$freePage->setContent(file_get_contents(dirname(__FILE__) . "/free/".SOYSHOP_TEMPLATE_ID."/delivery.html"));
				$page->setPageObject($freePage);
				$logic->updatePageObject($page);

			//特定商取引法に関する表示
			$page = new SOYShop_Page();
			$page->setName("特定商取引法に関する表示");
			$page->setUri("exchange.html");
			$page->setType("free");
			$id = $logic->create($page);
				$page->setId($id);
				$freePage = $page->getPageObject();
				$freePage->setTitle("特定商取引法に関する表示");
				$freePage->setContent(file_get_contents(dirname(__FILE__) . "/free/".SOYSHOP_TEMPLATE_ID."/exchange.html"));
				$page->setPageObject($freePage);
				$logic->updatePageObject($page);
			...
			$page = new SOYShop_Page();
			$page->setName("検索結果");
			$page->setUri("search.html");
			$page->setType("search");
			$id = $logic->create($page);
				$page->setId($id);
				$searchPage = $page->getPageObject();
				$searchPage->setModule("common_search");
			...
			//404ページ
			$page = new SOYShop_Page();
			$page->setName("ページが見つかりません");
			$page->setUri(SOYSHOP_404_PAGE_MARKER);
			$page->setType("complex");
			$page->setTemplate("notfound.html");
			$id = $logic->create($page);
