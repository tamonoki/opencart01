...
function soyshop_output_item($htmlObj, SOYShop_Item $item, $obj=null){
	...
	$imageSmall = $item->getAttribute("image_small");
	$htmlObj->addImage("item_small_image", array(
		"src" => $imageSmall,
		"soy2prefix" => SOYSHOP_SITE_PREFIX,
		"visible" => (strlen($imageSmall) > 0) ? true : false
	));
	...
	$imageLarge = $item->getAttribute("image_large");
	$htmlObj->addImage("item_large_image", array(
		"src" => $imageLarge,
		"soy2prefix" => SOYSHOP_SITE_PREFIX,
		"visible" => (strlen($imageLarge) > 0) ? true : false
	));
