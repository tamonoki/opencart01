...
class EditorPage extends WebPage{
	...
	function EditorPage($args){
		...
		$moduleId = (isset($_GET["moduleId"])) ? str_replace("/", ".", $_GET["moduleId"]) : null;
		$this->moduleId = htmlspecialchars($moduleId);

		$moduleDir = SOYSHOP_SITE_DIRECTORY . ".module/html/";
		
		$modulePath = $moduleDir . str_replace(".", "/", $moduleId) . ".php";
		$iniPath =$moduleDir . str_replace(".", "/", $moduleId) . ".ini";
		...
		$this->addLabel("module_name_text", array(
			"text" => (isset($ini["name"])) ? $ini["name"] : $moduleId
		));
		
		$this->addInput("module_name", array(
			"name" => "config[name]",
			"value" => (isset($ini["name"])) ? $ini["name"] : $moduleId
		));
		...
		$this->addLabel("module_example", array(
			"text" => "<!-- shop:module=\"html." . $moduleId."\" -->\n" . @$ini["name"] . "のモジュールを読み込みます。\n<!-- /shop:module=\"html." . $moduleId."\" -->"
		));
