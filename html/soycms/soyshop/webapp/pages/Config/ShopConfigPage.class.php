...
class ShopConfigPage extends WebPage{
    ...
    function buildForm(){
		...
		$taxModuleList = $this->getTaxModuleList();
		...
		$taxModuleLink = $this->buildTaxModuleLink($config->getConsumptionTaxModule());
		...
		$this->addModel("multiCategory_batch", array(
			"visible" => ($this->checkMultiCategory()==true)
		));
    ...
    function buildTaxModuleLink($pluginId){
    	try{
